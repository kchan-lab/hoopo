...
8. **認証・認可の自前実装は暫定。優れたライブラリ/サービスが出てくれば置き換える前提で境界を保つ**
   (2026-09-05 追記、オーナー方針): 自前にしたのは「LIFF の既存セッションでタップ→自動ログイン」
   「LINE ユーザー ID 以外を外部に預けない」「ランニングコストゼロ」「Web 標準 API のみ」の
   4 条件を同時に満たす既製品が現時点で無いためであり、自前であること自体が目的ではない。
   置き換えを容易にするため次を守る:
   - ID トークン検証は `IdTokenVerifier` インターフェース(`packages/line`)の裏に閉じ、
     アプリ側は `verifyIdToken` の差し替えだけで済む状態を維持する
   - セッションの発行・検証は `packages/api/src/session.ts` の 1 モジュールに閉じ、
     ルート側は `SessionPayload`(sub / role / teamId / exp)だけに依存する
   - 認可(保護者↔子どもの所有権、コーチの役割)は各ルートに散らさず共通ヘルパーに集約する
     (#12 以降で実装。ここが最も置換時に効く境界)
   - 評価済みの候補と見送り理由: Clerk = LIFF 既存セッションを使う経路が一級でなく
     OAuth リダイレクトが増える(原則2)、表示名・メール等が Clerk 側に保存される(原則4)、
     Next.js middleware 結合がヘッドレス方針と逆向き。Firebase Auth / Supabase Auth = LINE を
     ネイティブに持たず Custom Token 用のサーバー関数が要り自前検証と手間が変わらない。
     これらの条件が変わったら(LIFF 対応・PII 最小化・無料枠)再評価する
