...
1. **生年月日と身長を保存する(絶対原則 4 の改定)**: 承認者が 2026-09-13 に「保存する」を選択。
   ...
   REQUIREMENTS §3・§5.2・§7 は 128a(#131)で、プライバシーポリシー(portal の掲示文と
   docs/PRIVACY_POLICY.md)は画面文言と一緒に 128b(#132)で改定する。128a だけが
   development に入る時間帯は保存項目と掲示文が一時的に食い違うため、3 本を続けてマージする。
