...
- [x] 日程管理にカレンダー表示を追加(既存の行一覧と切り替え or 併置)
- [x] 日付の複数選択、曜日見出しでその曜日をまとめて選択/解除
- [x] 選択バー: プリセット選択 + 開始・終了・場所・備考 → まとめて追加
- [x] すでに練習がある日の区別と注意表示
- [x] E2E(曜日まとめ選択 → プリセット適用 → 登録、既存の行編集・発行が壊れていないこと)
- [ ] PR 作成 → CI グリーン → development へマージ(`Closes #142`)

## 仕上げ

- [x] docs/REQUIREMENTS.md §5.2 の日程管理の記述を更新(カレンダーからのまとめ登録とプリセット)
