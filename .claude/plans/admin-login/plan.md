...
10. **メール+パスワード認証は暫定実装。認証・認可は優れたライブラリ/サービスが出てくれば
    置き換える前提**(2026-09-05 追記、オーナー方針。詳細は liff-login/plan.md 設計判断8):
    パスワード周辺(リセット・試行回数制限 #65・ハッシュ強度の追随)を自前で抱え続ける意図は
    ない。管理者は #61 の LINE ログインを主経路にし、メール+パスワードは LINE が使えない
    ときの予備(または廃止)と位置づける方向で判断する。置換に備え、ハッシュ方式は
    `password.ts` に閉じ(保存形式が自己記述)、セッションは 1a と共用の `session.ts` に閉じる
