...
3. **適用タイミングは「リリース PR マージ直後・release: PR マージ前」**: マージで Vercel 本番
   デプロイが自動で走るため、新コードが参照するスキーマを先に揃える必要がある。マイグレーションは
   後方互換(additive)を原則とし、非互換変更は expand→migrate→contract に分割する方針を docs に明記
