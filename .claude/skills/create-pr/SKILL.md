...
- **マージ方式は行き先で使い分ける**(2026-09-14、#129 の CHANGELOG 重複から見直し。rebase は使わない):
  - **feat/xxx → development は squash**。squash コミットの件名は **PR タイトル**になり、それが
    そのまま CHANGELOG の 1 行になる。したがって **CHANGELOG の 1 行 = PR 1 本**で、
    読み手に見せたい説明は PR タイトルと本文に書く(ブランチ上の個々のコミットは履歴に残らない)
  - **development → main のリリース PR と release-please の PR は merge commit**。squash すると
    development 側の各 `feat:` / `fix:` が main の履歴から消え、release-please のバージョン計算が壊れる
  - ルールセットは development=squash のみ、main=merge のみ。リポジトリ設定も同じ組み合わせに揃える
- **PR タイトルは Conventional Commits の日本語**にする(`feat:` / `fix:` / `perf:` が CHANGELOG に載る)。
  ブランチ上のコミット件名も同じ流儀で書く(commit Skill)が、CHANGELOG に出るのは PR タイトルだけ
...
4. **PR 作成**: **承認者(ユーザー)の承認があってから** `gh pr create --base development` する。
   ...
   - タイトル = squash コミットの件名 = CHANGELOG の1行になる形式 **「<type>: <作業名>」**(Conventional Commits 準拠):
...
7. **マージ**: 承認者の指示があってから
   `gh pr merge --squash --delete-branch`(development 向け)でマージし、ローカル development を `git pull --ff-only` で追従
