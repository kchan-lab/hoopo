...
- **リリース手順の実際**: development→main のリリースPRは **merge commit** でマージする(squash すると development の各 PR の `feat:` / `fix:` が main の履歴から消え、release-please のバージョン計算が壊れる)。マージ後に release-please が「release: vX.Y.Z」PR を起票するのでそれをマージ → タグ+Release+CHANGELOG が完了。直後に main→development の back-merge PR を出して CHANGELOG / version.txt を development へ同期する
- **マージ方式は行き先で使い分ける**(2026-09-14、#129 の CHANGELOG 重複から見直し): **feat→development は squash**、**development→main のリリース PR と release-please の PR は merge commit**。ルールセットとリポジトリ設定の両方でこの組み合わせだけを許可する。したがって **CHANGELOG の1行 = PR 1本**で、行の文面は **PR タイトル**(squash コミットの件名)になる(`feat:` / `fix:` / `perf:` が載り、`docs:` / `chore:` / `ci:` / `refactor:` / `test:` は載らない)。ブランチ上の個々のコミットは squash されて履歴には残らない(PR ページには残る)ので、**読み手に見せたい説明は PR タイトルと本文に書く**
...
- PRレビューは Claude Code GitHub Action で自動化。指摘の採否は自分で判断
  - **`claude-review.yml` が走るのは Open 時 / Draft→Ready 復帰時 / `re-review` ラベルを付けた時のみ**(push
    のたびには走らせない。小さなpushで毎回フルレビューが走るコストを避けるため)。レビュー指摘を直した
    push を再レビューさせたいときは、その push の後に PR へ `re-review` ラベルを付け直す(ジョブが自動で
    ラベルを外すので、次の回でも同じラベルを再利用できる)。会話・質問だけなら従来どおり PR コメントで
    `@claude` を呼ぶ(`claude.yml`)
  - **レビューが1件もコメントを投稿せずに終わるとジョブが失敗する**(空振り検知)。プロンプト側で
    「指摘が無くても `gh pr comment` で要約コメントを最低1件投稿する(『問題なし』でよい)」ことを
    必須にしているため、docsのみのPR等で本当に指摘が無い場合も要約コメントは残る。ジョブが赤くなったら
    「本当に空振りだったか」を確認してから再実行する(#185で空振り→再実行→帯を使い切って衝突が戻る、
    という事故があったため。原因を確認せず機械的に `gh run rerun` しない)
