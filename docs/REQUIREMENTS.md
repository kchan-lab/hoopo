...
- 初回登録(3ステップ):
  ...
  - ②参加情報: 参加可能曜日(日〜土のボタン複数選択) / 参加できる時間帯(**選んだ曜日ごとに開始〜終了**。選んだ時点で 09:00〜12:00 が入り、「すべての曜日に同じ時間を使う」が既定で入。外すと曜日ごとに別の時間にできる) / コーチへの伝達事項(自由入力・任意) / 続柄(父・母・祖父母・その他の1タップ選択)。時間帯の欄には「ふだん参加できる時間の目安であり、日によって違ってよい」ことを添える(厳密な約束と受け取られると実態を書けなくなるため)
