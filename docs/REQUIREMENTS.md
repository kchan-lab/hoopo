...
- 予定表画像のレイアウト(#215。#204 の2カラムを見直し): **幅1080pxの縦1列・1日1行**。
  「1日1行」は現行アプリ「縦型カレンダー」から踏襲し、iPhone 16 程度のスマホで LINE の全画面表示にしたとき
  **拡大せずに1か月すべてが1画面に収まる縦横比(31日の月でも 1:2.0 以内)**にする。
  行の高さは固定で、同じ日に複数コマある日も行を伸ばさず、行の中に縦に積んでその日だけ文字を小さくする
  (高さ = ヘッダー + 日数 × 行 + フッター)。縦横比に収まる範囲で余りは文字サイズに回し、
  ヘッダー・フッターは行に高さを譲るため薄くする。
  1行に収まらない長い場所名は1行のまま末尾を「…」で省略する(折り返すと高さの計算とずれるため)。
...
- ~~予定表画像の細部デザイン(現行スクリーンショットの共有待ち)~~ → 現行アプリのスクリーンショットが共有され、
  体裁は §6「予定表画像のレイアウト」(#204、#215 で縦1列に見直し)で確定。以後の細部調整は未決事項ではなく通常のIssueで扱う
