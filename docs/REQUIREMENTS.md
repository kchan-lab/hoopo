...
- 初回登録(2ステップ):
  - ①子ども情報: 名前 / 呼び名(ひらがな・練習で呼ばれる下の名前) / 生年月日 / 身長(cm・80〜220の整数) / 性別(男・女) ※兄弟・姉妹の複数登録可
...
| 9 | 家族の設定 | 子どもの招待コード表示・コピー、連携済み家族の一覧、子どもの名前 / 呼び名 / 生年月日 / 身長 / 性別の修正(学年は生年月日から再計算) |
...
| 部員管理 | 名前/学年/生年月日/身長/性別/連携保護者数/伝達事項。行クリックで詳細(伝達事項全文・参加可能曜日/時間・招待コード)。詳細から名前 / 呼び名 / 生年月日 / 身長 / 性別を編集できる(学年は生年月日から再計算)。**年度更新**ボタン。**卒団した部員**(アーカイブ済み)は一覧の下に表示し、「データを削除」(二段階確認)で本人の記録を物理削除する(紐づく保護者がいなくなればその保護者も削除)。削除は実行ログ(audit_logs)に残す |
...
- children(id, team_id, name, nickname_kana[呼び名], grade[1..6], birth_date[任意・date], height_cm[任意・80..220], gender[male/female], coach_note, invite_code[全体一意・推測困難], photo_path[任意], status[active/revoked・自動認定], archived, archived_at)
