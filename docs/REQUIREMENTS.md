...
- 初回登録(3ステップ):
  - ①子ども情報: 姓 / 姓のよみ(ひらがな) / 名 / 名のよみ(ひらがな) / 呼び名(ひらがな・練習で呼ばれる下の名前) / 生年月日 / 身長(cm・80〜220の整数) / 性別(男・女) ※兄弟・姉妹の複数登録可。姓・名とそれぞれのよみは必須で各25文字以内、よみはひらがなのみ
    - **学年は生年月日から自動判定**する(4月2日〜翌4月1日生まれが同学年。基準日は Asia/Tokyo の今日)。算出した学年が1〜6年に入らない生年月日は登録できない
...
| 部員管理 | 名前(姓+名)/学年/生年月日/身長/性別/連携保護者数/伝達事項。並びは学年降順→姓のよみ→名のよみ。行クリックで詳細(伝達事項全文・参加可能曜日/時間・招待コード)。詳細から姓 / 姓のよみ / 名 / 名のよみ / 呼び名 / 生年月日 / 身長 / 性別を編集できる(学年は生年月日から再計算)。**年度更新**ボタン。**卒団した部員**(アーカイブ済み)は一覧の下に表示し、「データを削除」(二段階確認)で本人の記録を物理削除する(紐づく保護者がいなくなればその保護者も削除)。削除は実行ログ(audit_logs)に残す |
